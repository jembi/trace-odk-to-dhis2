...
if [ "$1" == "up" ]; then
    ...
    docker exec postgres bash -c "while ! nc -z postgres 5432; do sleep 5; done; su postgres -c 'psql postgres -f create_dhis_db_and_user.sql'"
    ...
    docker exec postgres bash -c "while ! nc -z postgres 5432; do sleep 5; done; su postgres -c 'psql postgres -d dhis -f dhis2.sql'"
